...
let fullName = runners.map((runner) => {
    return {'first name': runner.first_name, 'last name': runner.last_name};
});
...
// Problem 1
/******How many medium shirts do they need to buy? ***********/
let mediumShirts = runners.filter((runner) => {
    return runner.shirt_size==="M";
});

console.log(`We need to purchase ${mediumShirts.length} medium shirts.`);
// Problem 2
/****************Which companies are participating?*********/

let participatingCompanies = runners.map((runner) => {
    return runner.company_name;
});

console.log(participatingCompanies);


// Problem 3

/****Return 3-digit bib numbers connected to runners' last names and emails. */

let bibNumbers = runners.map((runner) => {
    return {'bib number': runner.id + 100, 'last name': runner.last_name, 'email': runner.email};
});

console.table(bibNumbers);